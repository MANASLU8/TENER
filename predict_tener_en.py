...
def make_predictions(dataset_for_loading, dataset_for_prediction, filename, folderpath, subset_name_for_prediction, output):
  if dataset_for_loading in CONLL_CORPORA:
      n_heads = 14
      head_dims = 128
      num_layers = 2
      lr = 0.0009
      attn_type = 'adatrans'
      char_type = 'cnn'
  elif dataset_for_loading == 'en-ontonotes':
      n_heads =  8
      head_dims = 96
      num_layers = 2
      lr = 0.0007
  ...
  encoding_type = 'bioes'
  name = 'caches/{}_{}_{}_{}_{}.pkl'.format(dataset_for_loading, model_type, encoding_type, char_type, normalize_embed)
  ...
  def load_data():
      # 替换路径
      data = get_dataset(dataset_for_loading)
      char_embed = None
      if char_type == 'cnn':
      ...
      word_embed = StaticEmbedding(vocab=data.get_vocab('words'),
                                   model_dir_or_name='ru' if dataset_for_loading in RU_CORPORA else 'en-glove-6b-100d',
  ...
  trainer = Trainer(data_bundle.get_dataset('train'), model, optimizer, batch_size=batch_size, sampler=BucketSampler(),
                    ...
                    use_tqdm=True, print_every=300, save_path=folderpath)

  load_succeed = trainer._load_model(model, filename)
  ...
  databundle_for_prediction = read_conll_dataset(dataset_for_prediction)
  databundle_for_prediction.rename_field('words', 'chars')
  subset_for_prediction = databundle_for_prediction.get_dataset(subset_name_for_prediction)
  std_subset_for_prediction = data_bundle.get_dataset(subset_name_for_prediction)
  #print(type(subset_for_prediction))
  #print(subset_for_prediction)
  ##print(type(std_subset_for_prediction))
  #print(std_subset_for_prediction)

  predict(model, subset_for_prediction, data_bundle.vocabs["target"], output)

if __name__ == "__main__":
  parser = argparse.ArgumentParser()

  parser.add_argument('--dataset_for_loading', type=str, default='en-ontonotes', choices=list(set(['en-ontonotes'] + CONLL_CORPORA + RU_CORPORA)))
  parser.add_argument('--dataset_folder_for_prediction', type=str, default='../data/conll2003ru-test')
  ...
  make_predictions(args.dataset_for_loading, args.dataset_folder_for_prediction, args.filename, args.folderpath, args.subset, args.output)
